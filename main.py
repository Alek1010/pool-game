...
# load images
cue_image = pygame.image.load("venv/Pool assets complete - cropped/Assets/cue.png").convert_alpha()
table_image = pygame.image.load("venv/table.png").convert_alpha()
ball_images = []
for i in range(1, 17):
    ball_image = pygame.image.load(f"venv/Pool assets complete - cropped/Assets/ball_{i}.png").convert_alpha()
...
#function for outputing text on the screen
...
def creating_balls(radius, position):
    body = pymunk.Body()
    body.position = position
    shape = pymunk.Circle(body, radius)
    shape.mass = 5
    shape.elasticity = 0.8
    # use pixot joint to add friction
...
for col in range(5):
    for rows in range(rows):
        pos = (350 + (col * (dia + 1)), 267 + (rows * (dia + 1)) + (col * dia / 2))
        new_ball = creating_balls(dia / 2, pos)
        balls.append(new_ball)

# cue ball
pos = (888, SCREEN_HEIGHT / 2)
cue_ball = creating_balls(dia / 2, pos)
...
# create pool table wall
poolWall = [[(88, 56), (109, 77), (555, 77), (564, 56)],
...
# function for creating pool table wall
def pool_wall(poly_dims):
...
# create power bar to show how hard cue ball will be hit
...
while run:
    ...
    for i, ball in enumerate(balls):
        for pocket in pockets:
            ...
            if ball_dist <= pocket_dia / 2:
                #check if cue ball has been potted
                if i == len(balls) - 1:
                    lives -= 1
                    ...
                    balls.remove(ball)
                    potten_balls.append(ball_images[i])
                    ball_images.pop(i)
    ...
    #check if all the balls have stoped moving
    taking_shot = True
    for ball in balls:
    ...
    if taking_shot == True and game_running == True:
        ...
        y_dist = -(balls[-1].body.position[1] - mouse_pos[1]) # it is negative because pygame Y cordinates increase down the screen
    ...
    if powering_up == True and game_running == True:
        force = force +100 * force_direction
    ...
    for i, ball in enumerate(potten_balls):
    ...
    if lives <= 0:
        ...
        game_running = False
   #check if all the balls have been potted
    if len(balls) == 1:
